public class Stat {
    //Make attributes to store the following information:
    //

    
    //make a constructor

    //Make a method following the contract
    //Name: findAverage
    //Purpose: find the average of an array of values
    //Input: array of doubles
    //Output: average of doubles

    //Make a method following the contract
    //Name: findStandardDeviation
    //Purpose: find the standard deviation of an array of values
    //Input: array of doubles
    //Output: standard deviation of doubles

    //write a run file that:
    //finds the average of {10.2, 5, 43.23, 17.3, 23.90, 53.432}
    //finds the standard deviation of the same set of values
}
